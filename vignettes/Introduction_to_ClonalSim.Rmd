...
# Write to VCF file
vcf_file <- file.path(tempdir(), "simulated_mutations.vcf")
toVCF(sim, output_file = vcf_file)
```
...
```{r pyclone}
pyclone_file <- file.path(tempdir(), "pyclone_input.tsv")
...
```{r sciclone}
sciclone_file <- file.path(tempdir(), "sciclone_input.tsv")
...
```{r csv}
df <- toDataFrame(sim)
csv_file <- file.path(tempdir(), "mutations.csv")
...
```{r benchmark_workflow}
# 1. Generate ground truth
set.seed(42)
sim_bench <- simulateTumor(
  subclone_freqs = c(0.2, 0.3, 0.5),
  n_mut_per_clone = c(50, 75, 50)
)

# 2. Export to VCF (ground truth)
vcf_bench <- file.path(tempdir(), "ground_truth.vcf")
toVCF(sim_bench, output_file = vcf_bench)

# 3. Get true positive set
true_mutations <- getMutations(sim_bench)
head(true_mutations[, c("Mutation", "Clone", "Type", "True_VAF", "VAF")])

# 4. Evaluate VAF estimation accuracy (true vs observed)
cor(true_mutations$True_VAF, true_mutations$VAF)
```
